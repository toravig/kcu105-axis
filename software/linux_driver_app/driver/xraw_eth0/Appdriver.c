...
#if 0
	pkts = kzalloc( allocPages * (sizeof(PktBuf*)), GFP_KERNEL);
	if(pkts == NULL)
	{
...
//		pbuf = kmalloc( (sizeof(PktBuf)), GFP_KERNEL);
      ...
      //  pkts[j] = pbuf;
		...
		if(j==0) {
            ...
            pbuf->pktBuf = dma_map_page(chann->ptr_dma_desc->dev, cachePages[j],(j == 0) ? offset : 0, pbuf->size,DMA_TO_DEVICE);		
...
//		spin_lock_bh(&chann->channel_lock);
	//spin_lock(&chann->channel_lock);
				...
				if(retval < XLNX_SUCCESS) 
				{
					...
					dma_unmap_page(chann->ptr_dma_desc->dev,pbuf->pktBuf,pbuf->size,DMA_TO_DEVICE);
					page_cache_release(cachePages[j]);
...
//	}
	        ...
	        WR_DMA_REG(gen_chk_reg_vbaseaddr,NW_PATH0_OFFSET + XXGE_TC_OFFSET ,0x50000000);
	}
      else
	{
